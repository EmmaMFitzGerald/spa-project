...
class MemoriesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create memory" do
    assert_difference('Memory.count') do
      post memories_url, params: { memory: { category: @memory.category, child_id: @memory.child_id, content: @memory.content, title: @memory.title } }, as: :json
    end
  ...
  test "should update memory" do
    patch memory_url(@memory), params: { memory: { category: @memory.category, child_id: @memory.child_id, content: @memory.content, title: @memory.title } }, as: :json
    assert_response 200
  end
